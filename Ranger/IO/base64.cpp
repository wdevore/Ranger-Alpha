...
namespace Ranger {
    ...
    void Base64::encode(unsigned char const *bytes, unsigned int length, std::string& encodedOut) {
        int i = 0;
        int j = 0;
        ...
        while (length--) {
            ...
            if (i == 3) {
                ...
                for(i = 0; (i <4) ; i++)
                    encodedOut += base64_chars[char_array_4[i]];
                i = 0;
            }
        }
        ...
        if (i)
        {
            ...
            for (j = 0; (j < i + 1); j++)
                encodedOut += base64_chars[char_array_4[j]];
